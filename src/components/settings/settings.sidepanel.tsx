...
const SettingsSidePanel = ({ isOpen: propIsOpen, setIsOpen: propSetIsOpen }: SettingsSidePanelProps = {}) => {
  ...
  // FIX: Use primitive values instead of object to avoid infinite loop
  ...
  // Set initial settings when panel opens
  useEffect(() => {
    if (isOpen) {
  ...
  // FIX: Use primitive values in dependency array
  useEffect(() => {
    if (isOpen) {
      // For now, we only track changes to darkMode as language changes are applied immediately
      setHasChanges(initialDarkMode !== darkMode);
    }
  }, [darkMode, initialDarkMode, isOpen]); // ← Fixed: using primitive value instead of object
  ...
  return (
    <>
      {/* SIDE PANEL */}
      <div 
        className={`fixed inset-y-0 right-0 w-full sm:w-[400px] ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-lg z-30 transform transition-transform duration-300 ease-in-out ${
      ...
      >
        {!showLanguageSelector ? (
          <>
            ...
            {/* Panel Content */}
            <div className="overflow-y-auto flex-1 pb-28 px-4"> {/* Extra bottom padding for save button */}
              {/* Profile Section */}
              <div className={`my-4 p-4 ${darkMode ? 'bg-slate-700' : 'bg-slate-100'} rounded-xl flex items-center justify-between`}>
                <div className="flex items-center">
                  <img src={MOCK_USER.avatarUrl} className="rounded-full w-12 h-12 mr-3" alt="Profile" />
                  <div>
...
const SidePanelItem = ({ icon, label, description, darkMode, onClick }: SidePanelItemProps) => (
  <button 
...
interface SidePanelToggleItemProps {
  icon: ReactNode;
  label: string;
  isActive: boolean;
  onToggle: () => void;
...
const SidePanelToggleItem = ({ icon, label, isActive, onToggle, darkMode }: SidePanelToggleItemProps) => (
  <div 
